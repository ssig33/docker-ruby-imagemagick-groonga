...
RUN wget ftp://ftp.u-aizu.ac.jp/pub/graphics/image/ImageMagick/imagemagick.org/releases/ImageMagick-6.9.0-0.tar.xz && tar xvf ImageMagick-6.9.0-0.tar.xz && cd ImageMagick-6.9.0-0 && ./configure && make -j4 && make install && cd .. && rm -rf ImageMagick*

RUN apt-get update && apt-get install libffi-dev -y && rm -rf /var/cache/apt/archives/* /var/lib/apt/lists/*
