...
RUN apt-get update && apt-get install aptitude wget -y && aptitude -V -D -y --allow-untrusted install groonga-keyring && apt-get update && apt-get install nginx libgroonga-dev groonga libssl-dev -y && apt-get build-dep imagemagick -y && apt-get build-dep ruby -y && apt-get clean && rm -rf /var/cache/apt/archives/* /var/lib/apt/lists/*
